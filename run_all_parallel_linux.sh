...
# Linux version - Run all fuzzing experiments in parallel with FULL ASAN+UBSAN
# Use this on your Linux VM
...
echo "======================================"
echo "Running All Fuzzing Experiments (Linux)"
echo "======================================"
echo "This will run for 1 hour total"
echo ""
echo "Experiments:"
echo "  - Part B.1: No seeds (AFL only)"
echo "  - Part B.2: With seeds (AFL only)"
echo "  - Part C: ASAN + UBSAN (FULL sanitizers)"
echo "  - Part D: Custom mutator"
echo ""
echo "Note: Using FULL ASAN+UBSAN for Part C (Linux-compatible)"
echo ""
...
echo "Setting up system for ASAN..."
...
echo "Starting Part C (ASAN+UBSAN - FULL) in background..."
AFL_USE_ASAN=1 AFL_USE_UBSAN=1 "${BUILD_DIR}/AFLplusplus/afl-fuzz" \
  -i "${BUILD_DIR}/seeds" \
  -o "${BUILD_DIR}/output-c-sanitizers" \
  -V 3600 \
  -m none \
  -- "${BUILD_DIR}/harness-c-linux" @@ /tmp/out-c.png \
  > "${BUILD_DIR}/output-c.log" 2>&1 &
PID_C=$!
echo "  PID: $PID_C (with FULL ASAN+UBSAN)"
...
wait $PID_C
echo "✓ Part C complete (ASAN+UBSAN detected bugs will be in crashes/)"
wait $PID_D
